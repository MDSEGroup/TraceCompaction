...
//	  {
//	    aprioriProcess process1=new aprioriProcess(Trace, 0);
//	    System.exit(0);
//	  }
//}
...
public class aprioriProcess {
	...
	// Parameter : htn is a hashtreenode (when other method call this method,it
	// is the root)
	...
	// : and minsup
	// -------------------------------------------------------------
	public void getconfig(int numtrans, int numItems, int minsup) throws IOException {
		...
		// number of transactions : number of objectstate/parameterset
		// oneline=data_in.readLine();
		// N=Integer.valueOf(oneline).intValue();
		N = numtrans;

		// number of items : number of values/number of paramtervalue
		// oneline=data_in.readLine();
		...
		// if its 2 or more itemset
			...
			for (i = 0; i < length1; i++) {
				...
				for (j = i + 1; j < length1; j++) {
					...
					newcand = new String();
					if (n == 2) // if depth = 2, then no formula to determine
							...
							// itemsets to be combined
					{
						...
						if (same) // if the first n-2 items are the same,
									// combine the sets
						{
...
//		int i;
	...
	public void getlargehash(int i, hashtreenode htnf, String transa, Vector<String> lis) {
		Vector<?> tempvec = new Vector<Object>();
		int j;
	...
	// Return :
	// -------------------------------------------------------------
	public void transatraverse(Trace trace, int n) {
	//	FileInputStream file_in;
		//BufferedReader data_in;
		//String oneline = new String();
		int i = 0, j = 0;
		...
		for (ObjectState o : trace.getObjectstate()) {
			st = createObjectstateString(trace, o);
			transa = new String();
			// oneline=data_in.readLine();
			//numRead++;
	...
	public void transatrahash(int i, hashtreenode htnf, String transa) {
		Vector<?> itemsetlist = new Vector<Object>();
		int j, len;
		//int lastpos;
		String d;
		...
		if (htnf.nodeattr == IL) {
			...
			for (j = 0; j < len; j++) {
				 ...
				 * System.out.println("Lastpos: " + lastpos); if (lastpos!=-1) {
	...
	// Return :
	// -------------------------------------------------------------
	public aprioriProcess(Trace trace, int minsup) throws IOException {
		...
		// calling method for objectset and values
		// getconfig(trace.getObjectstate().size(),trace.getValue().size(),
		// minsup);

		N = trace.getObjectstate().size();
		M = trace.getValue().size();
		...
		while (true) {
			...
			// System.out.println("C"+k+"("+k+"-candidate-itemset):
			// "+cande.candlist);
			...
			// System.out.println("Now reading transactions, increment counters
			// of itemset");
			transatraverse(trace, k);
	...
	public StringTokenizer createObjectstateString(Trace trace, ObjectState objstat) {
		String strobjvalue = "";
		for (Value v : trace.getValue()) {
			if (((LeafObjectState) objstat).getValue().contains(v))
				strobjvalue = strobjvalue + "1" + " ";
			else
				strobjvalue = strobjvalue + "0" + " ";
		}
		StringTokenizer st = new StringTokenizer(strobjvalue, " ");
		return st;
	}
};